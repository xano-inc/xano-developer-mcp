...
- **v1 branch**: The default branch that always exists. It cannot be created or deleted, and its label cannot be changed.
- **Live branch**: The branch serving production API requests. Can be changed with \`branch set_live\`.
...
Xano CLI commands are SPACE-separated (e.g. \`xano branch list\`), not colon-separated.`,

  ai_hints: `**Key concepts:**
- Branches are identified by label (string), not ID
- "v1" is the default branch and cannot be created, renamed, or deleted
- Live branch handles all production API traffic
- Use \`branch list\` to see all branches in a workspace
- Use \`branch set_live\` carefully - it affects production traffic

**Typical workflow:**
1. \`xano branch list\` - see available branches
2. \`xano branch create dev\` - create new branch (label is positional; clones from v1 by default)
3. Work on dev branch (workspace pull/push with -b flag)
4. \`xano branch set_live dev\` - promote to production

**Argument style:**
- \`branch create\`, \`branch edit\`, \`branch get\`, \`branch delete\`, and \`branch set_live\` take the branch label as a POSITIONAL argument, not a flag
- \`branch create\` accepts \`-s/--source\` for the source branch (default: \`v1\`)

**Workspace ID:**
- Most commands need a workspace ID
- Pass via \`-w/--workspace\` flag or configure in profile
- \`branch list\` accepts the workspace flag as a string; \`branch get\`/\`create\`/\`edit\`/\`delete\`/\`set_live\` require an integer ID
- Use \`xano workspace list\` to find workspace IDs`,

  related_topics: ["workspace", "release", "profile", "sandbox", "integration"],

  commands: [
    {
      name: "branch list",
      description: "List all branches in a workspace",
      usage: "xano branch list [options]",
      flags: [
    ...
    },
    {
      ...
      flags: [
        { name: "workspace", short: "w", type: "integer", required: false, description: "Workspace ID (uses profile workspace if not provided)" },
        { name: "profile", short: "p", type: "string", required: false, description: "Profile name to use" },
        { name: "output", short: "o", type: "string", required: false, default: "summary", description: "Output format: summary or json" }
      ],
      examples: [
    ...
    {
      name: "branch create",
      description: 'Create a new branch by cloning from an existing branch. The branch label is a positional argument. Cannot create a branch named "v1".',
      usage: "xano branch create <label> [options]",
      args: [
        { name: "label", required: true, description: 'Label for the new branch (cannot be "v1")' }
      ],
      flags: [
        ...
        { name: "color", short: "c", type: "string", required: false, description: 'Color hex code for the branch (e.g., "#ebc346"). Note: on `branch create`, `-c` is taken by --color; use --config (no short) for the credentials file path.' },
        { name: "workspace", short: "w", type: "integer", required: false, description: "Workspace ID (uses profile workspace if not provided)" },
        { name: "profile", short: "p", type: "string", required: false, description: "Profile name to use" },
        { name: "output", short: "o", type: "string", required: false, default: "summary", description: "Output format: summary or json" }
      ],
    ...
    {
      name: "branch edit",
      description: 'Update an existing branch (cannot edit the "v1" branch)',
      usage: "xano branch edit <branch_label> [options]",
      args: [
        { name: "branch_label", required: true, description: 'Branch label to edit (cannot edit "v1")' }
      ],
      flags: [
    ...
    {
      name: "branch set_live",
      description: "Set a branch as the live (active) branch for API requests. Affects production traffic immediately.",
      usage: "xano branch set_live <branch_label> [options]",
      args: [
        { name: "branch_label", required: true, description: 'Branch label to set as live (use "v1" to revert to the default branch)' }
      ],
      flags: [
        { name: "force", short: "f", type: "boolean", required: false, default: "false", description: "Skip confirmation prompt" },
        { name: "workspace", short: "w", type: "integer", required: false, description: "Workspace ID (uses profile workspace if not provided)" },
      ...
      examples: [
        ...
        "xano branch set_live v1 --force",
        "xano branch set_live production -f -o json"
      ]
    }
  ],
  ...
  workflows: [
    {
      ...
      steps: [
        ...
        "Create dev branch from v1: `xano branch create dev`",
        "Pull dev branch code: `xano workspace pull -d ./dev-code -b dev`",
        "Make changes and push: `xano workspace push -d ./dev-code -b dev`",
        "Test changes in dev environment"
      ],
      example: `xano branch list
xano branch create dev
xano workspace pull -d ./dev -b dev
# Make changes...
xano workspace push -d ./dev -b dev`
    },
    {
      name: "Feature Branch Workflow",
      description: "Work on a feature in isolation before merging",
      steps: [
        "Create feature branch from dev: `xano branch create feature-auth -s dev`",
    ...
    },
    {
      ...
      steps: [
        ...
        "Set staging as live: `xano branch set_live staging`",
        "Monitor for issues",
        "Rollback if needed: `xano branch set_live v1 --force`"
      ],
